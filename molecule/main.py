...
def main():
    ...
    test_dataset = TensorDataset(data_x[index[train_size:]], data_pos[index[train_size:]], data_y[train_size:])
    train_loader = DataLoader(train_dataset, batch_size=args.bs, shuffle=True)
    if args.method in ['dann', 'cdan', 'brot']:
    ...
    log.logger.info(f'{strftime("%Y-%m-%d_%H-%M-%S", localtime())}\n'
                    f'{"=" * 40} Domain Adaption {"=" * 40}\nBackbone: {args.model}; Dataset: {args.data}; Method: {args.method}; Train: {train_size}; Test: {test_size}\n'
    ...
    if args.method == 'brot': loss_func = losses.TripletMarginLoss()
    ...
    for epoch in range(0, args.ep):
        ...
        for src_x, src_pos, src_y in train_loader:
            ...
            if args.method in ['dann', 'cdan']:
                ...
                if args.ot == 'new':
                    ot_loss = pot_brot(mass_src, mass_tgt, M, reg1=1e3, reg2=1e3, y=src_y)      # calculate the OT loss
                else:
        ...
        for tgt_x, tgt_pos, src_y in test_loader:
            ...
            if args.data in ['qm7', 'qm8', 'qm9']:
                val_metric += l1_loss(pred[..., 0], src_y).item() / test_size * len(tgt_x)
            else:
                val_metric += mse_loss(pred[..., 0], src_y).item() / test_size * len(tgt_x)

        if args.data not in ['qm7', 'qm8', 'qm9']: val_metric = val_metric ** 0.5  # use RMSE
        if args.method == 'brot':
            log.logger.info(
                'Epoch: {} | Time: {:.1f}s | reg_loss: {:.3f} | ot_loss: {:.3f} | triplet_loss: {:.3f} | Val_Metric: {:.3f} | Lr: {:.3f}'.format(
                    epoch + 1, time() - t1, reg_loss, ot_loss.item(), triplet_loss.item(), val_metric, optimizer.param_groups[0]['lr'] * 1e5))
        else:
            log.logger.info('Epoch: {} | Time: {:.1f}s | Loss: {:.3f} | Val_Metric: {:.3f} | Lr: {:.3f}'
